...
| 메뉴 노출 여부      | menu displayed        | 메뉴 노출 상태를 나타낸다. 메뉴 노출 상태는 변경 가능하다.                      |
...
| 메뉴에 속한 상품의 개수 | menu product quantity | 메뉴에 속한 상품의 개수를 나타낸다.                                    |
...
| 배달     | delivery         | 주문 유형 중 배달 유형을 의미한다.                                                                 |
| 포장     | takeout          | 주문 유형 중 포장 유형을 의미한다.                                                                 |
| 매장 내 식사 | eat in           | 주문 유형 중 매장 내 식사 유형을 의미한다.                                                            |
...
| 서빙     | served           | 주문의 음식이 모두 나온 상태이다.                                                                  |
| 배달 중   | delivering       | 주문이 배달 중인 상태이다.                                                                      |
| 배달 완료  | delivered        | 주문이 배달 완료된 상태이다.                                                                     |
...
| 배달 주소  | delivery address | 배달 유형 주문의 배송지 정보이다.                                                                  |
...
| 한글명   | 영문명      | 설명                               |
|-------|----------|----------------------------------|
| 배달    | delivery | 키친 포스에서 발생한 배달을 나타낸다.            |
| 배달 주소 | address  | 배달 될 주소를 나타낸다.                   |
| 배달 가격 | sum      | 배달 될 주문의 가격을 나타낸다.               |
| 배달 가격 | order id | 배달 될 주문의 id 를 나타낸다. id는 겹치지 않는다. |
...
| 테이블 점유 여부 | occupied     | 매장 내 테이블의 점유 상태를 나타낸다.   |
| 테이블 손님 수 | num of guest | 테이블에 있는 손님의 수를 의미한다.     |
| 테이블 앉기   | sit          | 테이블에 앉는 행동을 의미한다.        |
| 테이블 앉기   | clear        | 테이블 이용을 마치고 치우는 것을 의미한다. |
